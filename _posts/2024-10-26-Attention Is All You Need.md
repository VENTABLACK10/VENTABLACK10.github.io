...
### 2. Introduction
* 기존 RNN, LSTM, GRU는 언어 모델링과 기계 번역에서 중요한 역할을 해왔지만, 순차적 특성으로 인해 긴 시퀸스를 처리할 때 병렬화가 어려운 문제와 순차적 연산의 제약이 존재했다.
* attention 메커니즘은 위의 제약을 극복하는 데 도움을 주었고, 이런 attention 매커니즘만 사용하여 본 논문에서는 입력과 출력 간의 전역적 의존성을 모델링하는 Transformer 모델을 제안했다.
